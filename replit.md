...
- **Automatic Batch Creation**: Batches are automatically created during inventory intake processes (donations, transfers, etc.) for items with `is_batched_flag=TRUE`. No manual batch management UI - batches are system-managed for data integrity and traceability.
